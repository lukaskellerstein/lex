...
`hook.lua` is the `UserPromptSubmit` hook. It runs under `nvim -l`, reads the
hook's JSON on stdin, finds every `<lex-place>` block in the prompt, and
appends one record per block to `~/.lex/<repo-slug>/links.jsonl`. It never
